...
const DELETE = async ({ request, params }) => {
  const { id } = params;
  try {
    const res = await fetch(`${BASE_URL_API}/zonas/${id}/`, {
      method: "DELETE"
    });
    if (!res.ok) {
    ...
    return new Response(JSON.stringify({ message: "zona eliminada correctamente" }), { status: 200 });
  } catch (e) {
    console.error(e);
    return new Response(
...
const PUT = async ({ request, params }) => {
  const { id } = params;
  console.log("reciebiendo el ID de zona para actualizar", id);
